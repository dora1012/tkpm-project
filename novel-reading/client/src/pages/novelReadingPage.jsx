...
const novelReadingPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchNovelContent = async () => {
        try {
            const response = await axios.get(import.meta.env.VITE_SERVER_DOMAIN + '/api-content/' +slug+ '/' +chapterNumber);
            console.log(import.meta.env.VITE_SERVER_DOMAIN + '/api-content/' + slug + '/' + chapterNumber);
