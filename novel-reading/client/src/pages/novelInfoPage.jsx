    ...
    const fetchNovelInfo = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_DOMAIN}/api/${slug}`
        );
      ...
      <div className="bg-coral-pink">
        <div className="container w-9/12 mx-auto flex gap-20 px-20 py-20 bg-white">
          ...
          <div className="book-info flex flex-col flex-wrap gap-7">
            ...
            <div>
              <span className="font-semibold">Mô tả truyện:</span> {description}
            </div>
            <div className="book-actions mt-30">
              <button className="btn-dark">
                ...
                <Link
                  to={{
                    pathname: `/${slugify(title)}/chuong-1`,
                    state: { authors: authors.join(", ") },
                  }}
            ...
            {chapterList.map((chapter, index) => {
              ...
              return (
                <a
                  ...
                  key={index}
                  className="mb-5 break-inside-avoid block hover:text-coral-pink"
                >
