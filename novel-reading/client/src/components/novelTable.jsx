...
const novelTable = () => {
    ...
    useEffect(() => {
      ...
      const fetchNovelTable = async () => {
          try {
              const response = await axios.get(import.meta.env.VITE_SERVER_DOMAIN + '/api/truyen-moi-cap-nhat');
                ...
                <tbody>
                        {novelData.slice(0, 30).map((novel, index) => (
                            <tr key={index} className="hover:bg-gray-100">
                                <td className="py-2 px-4 border-b border-r"><a href={`/${slugify(novel.title)}`} className="hover:text-coral-pink">{novel.title} </a></td>
                                ...
                                <td className="py-2 px-4 border-b border-r text-center">{novel.categories.join(', ')}</td>
                                <td className="py-2 px-4 border-b border-r text-center">{novel.chapter}</td>
                                <td className="py-2 px-4 border-b text-center"> {novel.updateTime}</td>
                            </tr>
                        )
                        )}
                </tbody>
